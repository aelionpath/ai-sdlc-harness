...
A developer may inspect status directly. Repository Agent Skills use `--json` as a workflow navigator and can repeat routine bounded `NEXT` actions without requesting approval after each command. The coding agent, not Harness, performs those actions; it must stop for human authority decisions, unavailable required information, consequential ambiguity, or `BLOCKED`.
...
The generated skill guides an already-running coding agent; it does not make Harness launch or orchestrate Codex, Claude Code, or Gemini CLI. The skill preserves human authority and tells the agent when routine progression must stop for the developer.
